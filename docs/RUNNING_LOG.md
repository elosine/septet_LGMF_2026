...
## §279. `1m.4.1` EVERY VOICE KNOWS ITSELF — built and verified with no MIDI; THE SHIELD held (2026-09-22, Fable)

**What prompted it:** his word after `/postclear` on Fable — *"build 1m.4 articulation all the way thru independantly as much as
possible."* PLAN § `1m.4`'s build order: 1m.4.1 → .3 → .4 → .5 → .6, one commit each, THE SHIELD in each; 1m.4.2 waits on his rack;
1m.4.7 on his word. This is the first.

**What was built.** Every technique entry of `sandbox/instruments.js` now carries, AT LOAD, `kind` (`pitched` · `key` · `fixed`) and
`loud` (`vel` · `mw`); every `key` entry its `keys` or the word `pending`. 339 voices, none missing:

| instrument | voices | pitched | key | fixed | vel | mw | shaped (`shape: "mw"`) | keyed | pending |
|---|---|---|---|---|---|---|---|---|---|
| english horn | 36 | 29 | 7 | 0 | 30 | 6 | 10 | 0 | 7 |
| bassoon | 22 | 19 | 3 | 0 | 22 | 0 | 0 | 0 | 3 |
| horn | 25 | 25 | 0 | 0 | 25 | 0 | 0 | 0 | 0 |
| trumpet | 35 | 35 | 0 | 0 | 35 | 0 | 0 | 0 | 0 |
| percussion | 32 | 1 | 31 | 0 | 32 | 0 | 0 | 31 | 0 |
| vibraphone | 13 | 13 | 0 | 0 | 13 | 0 | 8 | 0 | 0 |
| cello · double bass | 88 each | 52 | 8 | 28 | 65 | 23 | 10 | 0 | 8 |

- **Where the data comes from.** The Xsample rosters are GENERATED by their helpers, so the helper's `P()` stamps each entry: `kind` from
  a named set per roster (the strings' FIXED = the open strings and the natural harmonics, 28; their BY-KEY = tailpiece (2) · behind the
  bridge · peg box · finger · body · undefined (2), 8; the english horn's BY-KEY = multiphonics (2) · key noises · various noises · air
  noises (2) · undefined tones, 7 — §244's list) and `loud` FROM THE PRESET NAME: `Velocity` in the name = the velocity; a bare `MW` =
  the wheel; both (`Velocity + MW inverted`, `Velocity MW Shape`, `Velocity X MW`, `MW - Velocity`) = `vel` with `shape: "mw"` — the
  plan's [call], the wheel only shapes it. The three SI2 tables are typed by hand one line per preset and stay as they are: a
  `SI2_KINDS` table at the foot of the file names their by-key voices (the bassoon's blow without reed · key click · multiphonics; the
  horn and trumpet none on the manual's list — 1m.4.2 checks at the rack) and `applySi2Kinds` stamps the rest pitched, by velocity —
  ONLY for the three instruments it names, so a new instrument or a helper entry without `kind` is caught, never defaulted. The ARO
  percussion's generator (`tools/apply_perc.js`) now writes `kind: "key", loud: "vel"` into every generated technique and was re-run
  (the block's date line is the only other change). The hand-typed claves of 1l.1 are `key`; **`main` is `pitched` on purpose** — the
  placeholder takes any note it is dealt, and his harmony takes carry 241 percussion notes on it (§212).
- **The drawer.** `kindOf` already read `tech.kind` first (the port left that door open in 2026-09-03's code); what changed is the
  grouping the row menus and the picker use — a `KINDS` list, `pitched · fixed · key · noise · multiphonic`, so a `key` voice is shown
  (before this the menus would have dropped an unknown kind). `noise` and `multiphonic` stay as the name rule's older names for any
  entry without the field; they sound as `key` does — a stand-in, never the harmony pitch (`fitReal`: every kind but `pitched` takes
  the stand-in path; `defaultStandIn`: `fixed` the nearest open string, the rest the pitch folded into the range).
- **`tools/roster_check.js`** — a line per instrument (the table above), then: every voice has a kind and a loudness source · every
  key voice has its keys (`[{ midi, label }]`, 0–127) or is pending, and no other voice has keys · every instrument's ordinary voice is
  pitched · the pending list printed as a note. ROSTER GREEN: 3 checks, 339 voices, 26 pending. Joins the palette battery.

**What the name rule had WRONG, now set right by the data** (read off the drawer's own menus before and after, one row each):
the horn's `stopped_to_open` and the trumpet's four wah-wah voices were `fixed` (the rule's `_open` matched them — they are pitched);
the percussion's 31 catalogued instruments were `pitched` (no word of the rule matched a Spitfire name — they are by key); the strings'
behind-the-bridge and peg-box pizzicati were `fixed` (the rule's `behind_bridge|peg_box`) — the plan makes them `key`, the pitch
folded rather than the nearest open string. Menus before → after: english horn `pitched:29 noise:5 multiphonic:2` → `pitched:29 key:7`
· bassoon `pitched:20 noise:1 multiphonic:1` → `pitched:19 key:3` (blow without reed left `pitched`) · horn `pitched:23 fixed:1
noise:1` → `pitched:25` · trumpet `pitched:30 fixed:4 noise:1` → `pitched:35` · percussion `pitched:32` → `pitched:1 key:31` · the
strings `pitched:52 fixed:30 noise:6` → `pitched:52 fixed:28 key:8`. Nothing of this reached a note in his takes: the sets and the
deal use pitched voices throughout.

**REQUIRED VERIFICATION, run:** `roster_check` zero missing ✓ · `palette_check` 198 ✓ · **THE SHIELD** — on `score-5401` (no MIDI, every
POST and the beacon stubbed, `autosave` off, the eleven ports stubbed to a byte log): the strike mode on `cs-012` (7 notes), `may fold`
on, the shuffle at seed 5, the same routine before the edits and after a reload — `notesFor('orch')` with each note's route (port ·
channel · CC0 · keyswitch) under the deal and under each of the four sets, and the bytes `♪ as dealt` sends. **Byte-identical**: the four
sets, the MIDI (25 messages), and the deal — the deal matched only once the set in force was put back to `ordinario`, because the
before-run's own last `applyArtSet` (`percussive`) had been saved to the browser and came back after the reload; under the same set
the deal is the same to the byte. The one thing that LOOKS different in the strike mode is the group heading in the row menus and the
