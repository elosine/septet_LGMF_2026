...
- **1r — CONVERGE ON A TAKE: the take is the ARRIVAL — each player opens a semitone around their own just note and closes onto it; the vibraphones held still** — `planned` 2026-09-24 (Fable, session 14; RUNNING_LOG §322 the actual read · §323 his "a" and the calls; H1.6's *the next small build adds to this list*) —
  ...
  **The design (his "a"):** the take's just chord is the PURITY, cents and all (LG-27); the friction is the opening. Each player opens ONE semitone around their own assigned note — seat a above, seat b below, the bloom's way — and closes onto it over the run. A pair the drawer doubled onto one note does today's stock gesture in just intonation. No player is asked for more bend than they have. Rejected (b): the take as the START, the pair's two notes closing onto their mean — real, but only when pair-mates are within reach, and the line would have to refuse the rest.
  **The calls, his to reverse:** two stations, open → close, ending on the take (a dwell later if he wants it) · the opening the stock's, a semitone each way, no dial · the bloom's take rules unchanged (one alone → the partner doubles when it can, neither → the pair sits out, a player with two notes → its lowest) · the vibraphones in, held still on their assigned notes, as in the bloom (`1i`) · SPECTRAL (M2) and COLOUR (M4) stay refused — a fundamental is not a chord, and COLOUR holds pitch · an opened note outside the player's compass opens on the OTHER side; both sides out → it stays on its note, said in the line.
  ...
  - **1r.5 — REQUIRED VERIFICATION** (`score-5401`, journal §2 STILL BINDING's recipe: the stubs in the navigation batch, `Composer.autosave` stubbed and again after any `openScore`, never Saved, every port stubbed, `resize_window` 1280 × 860 first):
    ...
    (iv) CONVERGE + `Just-c2-seed143` → Generate: `source.voices` opened (a +1, b −1, cents kept), `target.voices` the take's notes, `target.midi` gone, the vibraphones `still` with source = target, `lanes` with lane 5 twice; the render's first sample per voice at the opened cents and its last at the take's (within `clampCents`); the still voices flat; the line as 1r.3; the doubled case (a take with one note on a pair) opens both ways from it;
