...
- **1r — CONVERGE ON A TAKE: the take is the ARRIVAL — each player opens a semitone around their own just note and closes onto it; the vibraphones held still** — **`built` 2026-09-24 (Fable, at his word *"build here go"*; RUNNING_LOG §325; one commit; every check of 1r.5 ran; `1r.6` his one test next — a reload only)** — `planned` 2026-09-24 (Fable, session 14; RUNNING_LOG §322 the actual read · §323 his "a" and the calls; H1.6's *the next small build adds to this list*) —
  **His words:** *"it says vibs in a bloom take only, this is converge can I get the vibes there as well?"* · *"lets get converge to accept a take"* · to the one question, where the take sits: **"a"**. What he likes in `ACT-CONVERGE-01` (`lgmf-s03-convergeA`) is the STOCK CONVERGE (§322) — three whole-tone pairs closing onto D♯3 · G♯3 · C♯4 over 107 s, two stations, no dwell; the take on the pulldown was refused under M3 and contributed nothing.
  **The design (his "a"):** the take's just chord is the PURITY, cents and all (LG-27); the friction is the opening. Each player opens ONE semitone AWAY FROM THEIR PARTNER — the higher player of the pair up, the lower down (his word 2026-09-24, RUNNING_LOG §324: the seat order then decides nothing; a doubled pair a above, b below, the bloom's way) — and closes onto their own assigned note over the run: every pair starts a whole tone wider than its interval in the take and tunes in. A pair the drawer doubled onto one note does today's stock gesture in just intonation. No player is asked for more bend than they have. Rejected (b): the take as the START, the pair's two notes closing onto their mean — real, but only when pair-mates are within reach, and the line would have to refuse the rest.
  **The calls, his to reverse:** two stations, open → close, ending on the take (a dwell later if he wants it) · the opening the stock's, a semitone each way, no dial · the bloom's take rules unchanged (one alone → the partner doubles when it can, neither → the pair sits out, a player with two notes → its lowest) · the vibraphones in, held still on their assigned notes, as in the bloom (`1i`) · SPECTRAL (M2) and COLOUR (M4) stay refused — a fundamental is not a chord, and COLOUR holds pitch · an opened note outside the player's compass opens on the OTHER side; both sides out → it stays on its note, said in the line.
  - **1r.1 — The door.** `score/public/morph_panel.js` `TAKE_MODELS: ['M1', 'M3']`. The refusal (`applyPitch`'s `refused` branch and its note, *"a take is read by BLOOM only so far"*) names the list generically: *a take is read by BLOOM · CONVERGE so far (M1 · M3) — under X the model's own set plays*. The pairs row's still label `held still · in a bloom on a TAKE only` → `held still · on a TAKE (BLOOM · CONVERGE)`. Nothing else changes for M1. **As built (§325): the LG models are engine M3 too, their voices in their base params — `namesOwnVoices(key)` gates the take path AND the recall branch, so LGSPECTRAL · LGBLOOM · LGCONVERGE refuse a take with a line of their own and their eighteen actuals recall exactly as before.**
  - **1r.2 — The stations.** In `takeVoices(params)`, after `out.source` is built (~1642): when `params.model === 'M3'` —
    `out.target = Object.assign({}, out.target || {}, { kind: 'voices', voices: <the take's notes, one per voice, in the source's order — midi · cents · partial · seat · still as they are> })` is the ARRIVAL, and `out.target.midi` is deleted (the stock's `[51, 51, …]` must not linger beside it);
    `out.source.voices` becomes the OPENED chord: of a pair's two voices the HIGHER (by `midi * 100 + cents`) `{ midi: m + 1, cents }`, the LOWER `{ midi: m − 1, cents }` — away from the partner (§324); a doubled pair (one note on both) seat a `+ 1`, seat b `− 1`; a `still` voice is NOT opened (source = target = its note — the engine's STILL keeps the start anyway, but Hear's first sample and the line must agree); a doubled pair opens from its one note both ways. The range rule of the calls (`env.BC.holds(env.recipe, inst, midi)` on the opened note; the other side; else unopened, `row.warn`, a `TAKE:` warning).
    The engine reads it as it stands: `targetCents` prefers `target.voices` (morph.js ~1351, in the source's order), `M3` interpolates start → target over the run, two stations (no `mid`, no `dwell` written). **`morph.js` and `morph_septet.js` are untouched** — the fourth use of the `voices` door (`kind: 'voices'` · `fadeWeight`'s `to` · `still` · this). `lanes` unchanged.
  - **1r.3 — The line.** H2.4's pair-by-pair line under M3: the head reads *as assigned — the ARRIVAL; each pair opens a semitone each way and closes onto it*; each bending row's `why` gains `· opens ±1 st → closes onto it` (a still row unchanged: `held still`). The refusal text of 1r.1.
  - **1r.4 — The actuals.** Nothing to build if H2.6's recall holds with M3 in the list (`~961`: `TAKE_MODELS.indexOf(rp.model) >= 0 && lanes.length === voices.length`): a recalled CONVERGE-on-a-take comes back with `source.voices` (opened) and `target.voices` (the take) and re-renders identically. The store's schema already carries `target.kind: 'voices'` (the LGMF actuals). Verified by the round trip in 1r.5, not assumed.
  - **1r.5 — REQUIRED VERIFICATION** (`score-5401`, journal §2 STILL BINDING's recipe: the stubs in the navigation batch, `Composer.autosave` stubbed and again after any `openScore`, never Saved, every port stubbed, `resize_window` 1280 × 860 first):
    (i) `node tools/model_bank.js --validate` before and after — all 31 stored actuals byte-identical (THE SHIELD's shape: the only door is `TAKE_MODELS`);
    (ii) CONVERGE with NO take (the stock set) → `resolvedParams` identical to `ACT-CONVERGE-01`'s: source `[50, 52, 55, 57, 60, 62]`, target `[51, 51, 56, 56, 61, 61]`;
    (iii) BLOOM + `Just-c2-seed143` → params byte-identical to HEAD's (`git stash` the change, capture, pop, capture);
    (iv) CONVERGE + `Just-c2-seed143` → Generate: `source.voices` opened AWAY from the partner (the higher +1, the lower −1, cents kept — his case, §324: Vc F4 −14 ¢ · Db B2 · Tpt G5 −49 ¢ · Hn B4 · EH C♯4 · Bsn B1; the same with pair 1's menus swapped), `target.voices` the take's notes, `target.midi` gone, the vibraphones `still` with source = target, `lanes` with lane 5 twice; the render's first sample per voice at the opened cents and its last at the take's (within `clampCents`); the still voices flat; the line as 1r.3; the doubled case (a take with one note on a pair) opens both ways from it;
    (v) Hear captured: zero note-ons on MAIN ch 1, every voice on a curve channel, the vibraphones on `LGVibes` ch 2 · 3 · 4 with bend at centre, `velAbs` the instrument's mf (the law, `1h` H3);
    (vi) Insert → objects on the six lanes + lane 5, `velAbs` · `cc7Abs` on every one, the vibraphone objects' `morphBend` flat, `Composer.curveDirty()` called;
    (vii) Save as ACTUAL `zz-1r` → recall → Generate → identical params and notes; the test actual deleted after and `bank/` byte-identical.
  - **1r.6 — His one test:** reload the tab — page files only, **no restart** (`morph.js` is untouched, so the server's engine copy is not stale). CONVERGE · `Just-c2-seed143` on the pulldown · Generate → the line reads *as assigned — the ARRIVAL* · Play · Save as ACTUAL if he keeps it. Revise on his word.
  - **THE BUILD ORDER:** 1r.1 → 1r.4 in ONE commit with 1r.5 — Opus, at his word; 1r.6 his.

- **1s — THE MORPH BETWEEN `min` AND `max`: the level in written dynamics, the morph's own shapes running between them** — **`built` 2026-09-24 (Fable, at his word *"go direct to plan and build pls"*; RUNNING_LOG §329; one commit; `1s.6` his one test next — a reload only)** — `planned` 2026-09-24 (Fable, session 14; RUNNING_LOG §326 the diagnosis · §327 the morph's level read to him · §328 his "A" and the top line) —
  - **The design line:** his ear, *"the max vol seems quite loud no matter how I lower dyn amount. can we have it move between a min and max level"*. `dyn amount` was the HALF-SWING round `dyn.base`, a centre with no box (CONVERGE 0.5, the engine 0.6), so lowering it narrowed the band round mf and never lowered the top. Now the panel shows **`min` and `max` in written dynamics** — the table's names `ppp … fff`, the sequence drawer's waves idiom (`low · high`) — and the engine's dials are DERIVED: `base = (min + max) / 2`, `amount = (max − min) / 2` on the table's 0 … 1 level. **`morph.js` untouched** (as `1r`): `dynLevel` reads `base ± amount × w`, `w` −1 … +1, so the trough IS `min` and the peak IS `max` under every shape. **The same `pp` as everything else** — one table (`dyn_table.js`, DYNAMICS_LAW §3 Rule 2 · 3), `fff` = 127, 4 dB a step, each instrument's measured curve. **[call] A, his (§328):** the morph's own five shapes only; a `waves` shape (the sequence drawer's generator per player between `min` and `max`) is NOT built — feasible in the panel later, since it already rewrites every note's nodes.
  - **1s.1 — The two boxes:** `min` · `max` menus on the panel in place of `dyn amount` (`morph_panel.js` `drawDyn`); two HIDDEN number boxes carry `dyn.base` · `dyn.amount` at full precision and `readFields` reads THEM (a `select` with a `data-path` would write a string) — the menus only WRITE them, both dials from the two names shown, the lower name the trough whichever menu holds it. Under `flat` one menu, `level`, writes `base` alone.
  - **1s.2 — The shapes between them:** nothing to build — the engine's `swell` · `rise` · `fall` · `rotate` all swing −1 … +1, so they run trough = `min`, peak = `max`; `spread` still staggers the parts; the release still fades below `min` at the end (its own layer, 2z). **Known and accepted:** the engine's level floor is 0.4 of 10, so `min ppp` plays about 1 dB over the table's `ppp`. **Found in 1s.5 (§329), his call:** a model's own level dips (CONVERGE's soft entries) go UNDER `min` — they start at the engine's floor 0.4 of 10 on HEAD too (18 of `ACT-CONVERGE-01`'s 88 breaths); one clamp in `morph.js` would make `min` a hard floor and move every stored render.
  - **1s.3 — The stored models and actuals:** every one recalls and regenerates byte-identical — the hidden boxes are drawn from the stored `base` · `amount` exactly, the menus show the NEAREST name with `≈` when the dial sits between names, and write only when he changes one (a recalled `0.5 ± 0.4` snaps to `pp … ff` the first time he touches either menu, never before).
  - **1s.4 — The status line** reads the names back beside the CC7, on Play and on Insert: `… the fader CC7 51…109 · min pp · max ff` (`dynText`); `flat` reads `level mf`.
  - **1s.5 — REQUIRED VERIFICATION** on `score-5401` (STILL BINDING's recipe, 0 POSTs): (i) `node tools/model_bank.js --validate` unchanged · (ii) THE SHIELD — the stock CONVERGE and BLOOM + `Just-c2-seed143` rendered on HEAD's file and on the build's, params and notes hash-identical · (iii) `min pp · max ff` on his CONVERGE + `Just-c2-seed143` → `dyn.base` 0.5 · `dyn.amount` 0.357…, every rendered note's level within 1/7 … 6/7, every inserted note's `cc7Abs` inside its instrument's table `pp … ff`, the status line naming both · (iv) `flat` + `level mf` → every note flat at the table's `mf` · (v) Hear's status = Insert's (the same CC7 span and names) · (vi) a recalled actual's menus read `≈` where its dials sit off the names and its render is byte-identical.
...
- **1t — THE MORPH BETWEEN TAKES: take A → take B, each player sliding from their own note to their own note, the vibraphones switching** — **`built` 2026-09-25 (Opus, session 15, at his word *"a and build as much as possible independantly"*; RUNNING_LOG §364 … §367; four commits, THE SHIELD in each; `1t.5` his listen next — RESTART the server, reload the tab). Found in the build: the switch needs a CUT at its moment (§366); the engine's release sent every player back to take A — `carrier.releaseHolds`, opt-in, on TAKES (§367)** — `planned` 2026-09-25 (Fable, session 15, at his word *"good to write whole plan, skip step by step, check in before build"*; RUNNING_LOG §350 … §362; COMPOSITION_NOTES LG-106 … LG-110; MORPH_NOTES §3 the same day) — ***why:*** the rondo's refrain is a morph and his harmony lives in TAKES; until now a take could only be a bloom's or a converge's chord — the transition from one take to the next, the piece's joints, had no tool. *"I want to use the morph to transition between 2 takes."*
  - **THE REQUIREMENTS (the concept, closed §360):** TWO takes, A → B (a series is the rebuild's) · each player from THEIR note in A to THEIR note in B, cents kept · ONE typed duration = the whole: the first leaves A at 0, the last arrives at B at its end, the starts scattered by the seeded shuffle inside the `spread` window, no two together · a distant pitch by the engine's SEGMENTED RE-KEY as it stands (§352) · no pacer — the duration is the pace · the breaths as `1j` (own max ± jitter, the outlier) · the ordinary long tone · `min` · `max` as `1s` · the fade = `release`, tacked on · the entry as today, staggered inside half a breath · a player in A with no note in B: a FLAG, the strikes drawer the fix (§351) · the vibraphones: a dial 0 … 1 of the duration for the switch, a RE-STRIKE never a bend, the two seats a few seconds apart (§354) · the joins with the sequences BY HAND (§359: `exit: one by one` before, `enter: seamless` after, a drag where a gap looks wrong).
  - **1t.1 — The model in the bank:** a stock entry `TAKES` — *"TAKE → TAKE"* — in `bank/morph_models.json`: `modelType 'M3'`, no `target.mid`, no `dwell` (the two-station form, `morph.js` M3 425 … 428 the plain interpolation); its `baseParams` WITHOUT `source.kind: 'voices'`, so `namesOwnVoices` is false and the pulldown's take is read; `carrier` span 60 · `duration` null · `release` null · segLen 12 · segVar 0.3 · striation `staggered` · the `1j` dials on (`ofMax 0.65 · jitterS 1.3 · outlier {0.1, 0.4, 2}`); `dials` spread 0.15 · bias 0 · depth 1; `dyn` the `1s` standard (`min pp · max ff`); a character line and the recipes it needs (none at first). `TAKE_MODELS` already holds `'M3'`. `node tools/model_bank.js --validate` green; every existing model and actual untouched (THE SHIELD).
  - **1t.2 — The `to` take on the panel, the arrival by player, the flags, the one duration box** (`morph_panel.js` alone): under `TAKES` the PITCHES pulldown is `from ▾` and a second pulldown `to ▾` sits beside it — the same takes menu (the sequence drawer's `opts`, `▸` hears without choosing). `takeVoices` builds the START from take A as assigned (cents kept, the vibraphones by `lane:seat` as `1i`) and the TARGET from take B by the SAME `lane:seat` mapping — **`1r`'s "opens a semitone away" is keyed on the model TYPE today (`const arrival = M3 ? …`, 1724); it moves to the CONVERGE key, so `TAKES` reads both takes plain.** A player in A with no note in B: target = start (holds) and the line FLAGS *`Bsn — no note in "to"; holds`*; a player in B with none in A: left out, flagged. THE TIME BOXES under this model: ONE box `duration (s)` writing `carrier.span`; `carrier.duration` hidden and forced null (a body longer than the gliss FOLDS everyone back toward A — `carrierTiming` 296, `foldPhase`; §361); `release (s)` stays as the fade; every other model's panel unchanged. `spread` stays the scatter. Both takes' names carried in `info` for the line and the actual.
  - **1t.3 — The vibraphones' switch — engine + dial:** `morph.js`: `switchAt` on a voice (0 … 1 of the travel), the sibling of `still` in `stateAt` (1331 · 1663) — before `switchAt × travel` the voice's start cents, after it the target's, no `motionDev` (a step); ADDITIVE AND OPT-IN — with no `switchAt` on any voice not a byte of any stored render moves. The step is turned into a CUT and a RE-STRIKE on the new key by the adaptive sampling + the segmented re-key (the struck reach is nil, the bowed 33 c; 1719 … 1744) — no new emit path; verified in the capture, not assumed. Panel: a row `vibes switch 0 … 1` (default 1 = at the arrival, his first try; 0.5 his second); seat 0 at the dial, seat 2 at the dial + 3 s (the AI's call), clamped to the travel; under `TAKES` the vibraphones are NOT `still`. Restart the server after (the server's engine copy, journal §2 standing warnings).
  - **1t.4 — The line, the actual, the required verification:** the status on Play and Insert: *`take A → take B · 8 voices as assigned · flags … · vibes switch 60 s · 63 s · the fader CC7 … · min pp · max ff`*. Save as ACTUAL carries `source.voices` + `target.voices` + `info {take, to}` so a recall renders byte-identical and its line names both takes (H2.6's rule); Insert = Hear (one render). **REQUIRED VERIFICATION** on `score-5401` with the stubs of STILL BINDING (journal §2), every ONE captured: (a) THE SHIELD — every stored model and actual re-rendered byte-identical before and after each step (`model_bank --validate`; the LG actuals; the tuba goldens' probe as 2c's recipe) · (b) a transition on two of his takes: every voice's first note at A's cents and its last at B's, to the cent; the last arrival at the duration's end; no two starts equal · (c) a voice with ≥ 5 semitones to travel: re-keyed with no gap, every seam inside the reach · (d) the vibraphones: ONE note-on on the new key at the dial's time per seat, seat 2 three seconds later, no bend past the reach · (e) the flag on a take missing a player, the voice holding · (f) the saved actual recalled byte-identical, its line naming both takes · (g) the routing on the law: MAIN ch 1 empty, every voice on a curve channel at its mf velocity with the fader between the table's `min` · `max` (as `1r.5`'s checks).
  - **1t.5 — His listen:** RESTART the server (`morph.js` changed) and reload the tab. MORPH → `TAKE → TAKE` → `from` · `to` two of his takes → `duration` 60 → Generate → the line → Play → the seams on a far voice · the vibraphones' switch at the arrival, then at 0.5 · Insert after a sequence ending `one by one`, the next sequence `seamless`. Revise on his word — a design question on Fable, a fault on Opus.
  - **THE AI'S CALLS, his to reverse:** the seat offset 3 s · the default switch at the arrival · the fold box hidden · a missing target holds · `staggered` kept (`aligned` withdrawn, §358) · the joins by hand. **THE REBUILD'S (MORPH_NOTES §3, not here):** a series of takes · the per-part join that reads the score (§358) · the cycles (§351) · the schedule drawn before Hear.
  - **THE BUILD ORDER:** 1t.1 → 1t.4 on Opus after a checkpoint + clear, one commit per step, THE SHIELD in each, RUNNING_LOG § per step; STOP before 1t.1 for his word (*"check in before build"*); 1t.5 his.

- **1u — THE VIBRAPHONES' PITCHES ON THE STRIP: the two bowed vibraphones in a placed sequence or morph change pitch at their breaths, the bowing untouched, from their take's own series, with a `back`** — **`built` 2026-09-26 (Opus, session 16, at his word *"go, build as much as possible independently"*; RUNNING_LOG §390 … §394; `f5bb5b9` · `fb779c2` · `a1b0e3d` · `ae1ee5d` · 1u.5; THE SHIELD 13 / 13; `vibes_pitch_check` 50; 1u.6 his — a reload only. The AI's readings, his to reverse, §391: a seat HOLDS its note until a change · a breath of another take is an anchor too · the other seat kept clear over the whole breath)** — `planned` 2026-09-26 (Fable, session 16, at his word *"Go ahead and write the whole plan. No need for steps."*; RUNNING_LOG §388 the talk · §389 the item; COMPOSITION_NOTES LG-114) — ***why:*** in every placed sequence and morph the vibraphones hold ONE note each the whole way (`1i` *held still*; `1t`'s switch is one re-strike). Their breaths are the only moments a pitch can move, and nothing moves it. *"let them play other notes as well. We'll keep the same breathing or bowing, but they'll just switch their pitches"* — and *"make sure I can get back, in case I don't like it."*
  - **THE REQUIREMENTS (closed §388, his answers *yes · b · all good · a · b · all three*):** the change LIVES ON THE NOTES in the score — `1q`'s way, the harmony strip; a sequence, a morph and any held passage alike, all notes by then; `morph.js` · `sequence.js` untouched · **get back = `back` + CTRL+Z** · THE POOL = the passage's own take's harmonic series, inside the vibraphone's range, under its tolerance (`spectrum_ui.js` `mayTake`: **|cents| ≤ 5**, a player who cannot bend), minus what the other seat holds — a dial of three: `neighbours` (the partial one above and one below each partial the others hold at that moment) · `within a tone` (every partial inside a whole tone of any other player's pitch then) · `any` (the whole series) · WHICH BREATHS: a dial `never · rarely · half · often · always` (0 · 0.2 · 0.5 · 0.8 · 1 per breath), a change always a DIFFERENT pitch when the pool allows · THE DRAW: `random` · `exhaust` · `walk` · `shadow`, one seeded stream, a seed reproduces (`1q.7`'s idiom) · THE TWO SEATS from one pool, KEPT APART — never the pitch the other holds at that moment; unisons with the other players allowed · the tolerance SHOWN on the strip, not a dial · the caveat accepted: a sequence re-inserted from its recipe regenerates and the change is gone (the status counts hand changes).
  - **THE SCENARIO (his, read back and confirmed):** CTRL+drag a marquee over the vibraphone lane across the passage (both seats fall in; other lanes in the box are ignored by this control) → the strip appears, names the take → `vibes ▾` opens one row: `pool` · `change` · `draw` · `seed` · `±5¢` · `go` → the selected breaths walked in time order, a new pitch at each change point → the status says what happened → play → `back`, CTRL+Z, or another seed.
  - **1u.1 — The pool** (`score/public/vibes_pitch.js`, NEW — a mixin on `HarmonySel` as `texture_cols.js` is on the drawer; its pure part loadable in node; `harmony_sel.js` gains the mount point only): **the take** of a breath, resolved in this order — `hq.take` on the note (a `1q` write) · the sequence's recipe in the score (`databases.sequences`, the note's group → the box at the breath's start → `box.take`; a sequence of several takes gives each breath its own) · a placed morph's stored actual (`info.take`, `info.to` after a TAKES switch — the marker's slug) · the `performanceNotes` fragment (`name · box N · TAKE · partial n`, which `HarmonySel.info` does not parse today — it reads only `← take "…"`) · else the strip's `take ▾` pick, the status saying *no take on these notes — pick one*. **The series** = `H.pool(name)` as it stands (`D.voices` through `dealTake`: every partial with its cents and number). **The range** = `H.range(o)` for the note's own technique. **The tolerance** = `D.mayTake(v, lane)`, not a copy of the number. **The seats** = the lane's two CHAINS by overlap — the selected vibraphone notes sorted by start, each joined to the chain whose last note ended before it (two colours; a score object carries no seat: the strip's `apply` alternates by time, §312) — the AI's call, his to reverse. **What the others hold at a moment t** = every OTHER pitched lane's objects overlapping t: their partial by `HarmonySel.info(o).partial`, else the series' partial nearest their `sonifyNote` + bend within 20 ¢ [call], else ignored. **The three rules** as the requirements; every pool recomputed at each breath (`any` is constant); the other seat's pitch at t removed. **`node tools/vibes_pitch_check.js`** (the `sequence_check` idiom): on the B1 series with his screenshot's assignment (EH 11 · Bsn 1 · Hn 6 · Tpt 15 · Vc 10 · Db 4 · Vib 16 · 9) — `any` = every partial under ±5 ¢ inside the range (8 · 9 · 12 · 16 · 17 · 18 · 19 and the rest the range allows; 10 at −14 and 13 at +41 OUT, as he expected) · `neighbours` and `within a tone` as computed, printed, the numbers pinned into the check once read · a pool of one leaves the second seat waiting · the take resolved on a sequence note, a morph note and a `1q` note.
  - **1u.2 — The draw:** the selected breaths of each seat walked in time order; **the first breath of each seat is left as it stands** (the take's own deal is the passage's anchor; `always` = every breath after it) [call]; at each breath one seeded draw (`mulberry32`, the strip's copy) against the `change` probability; on a change the pool of that moment minus the current pitch minus the other seat's pitch at that moment, then: `random` a uniform draw · `exhaust` a shuffled cycle PER SEAT over the pool, walked, reshuffled when spent (a member held by the other seat is skipped and kept) · `walk` the pool sorted by pitch, one step up or down from the current pitch's place, the direction by the stream, a bounce at the ends, a step over the other seat's pitch · `shadow` the member nearest in cents to any other player's pitch at that moment, ties by the stream, the current pitch excluded. An empty pool at a breath = no change, counted in the status. **The write** = the strip's own path: `remember(o)` once (`hq.was`, §307) · `writeNote(o, n, name)` with cents 0 (the vibraphone plays the tempered note, spectrum_ui §4; a channel bent before is re-centred by the existing rule) · `stamp(o, name, n, seed)` + `hq.seat` (0 · 2) for the card · `C.pushUndoState()` once for the whole `go` · `curveDirty` · `markDirty` · `rerender`. **THE SHIELD:** `harmony_sel.js` `apply` · `shuffle` · `back` byte-identical on the §317 · §319 captures of his `pointilistic01a` (the take, the shuffle, `back`) — a copy on `score-5401`.
  - **1u.3 — The strip:** a second line under the strip's first, shown only when the selection holds vibraphone-lane notes (`TRACKS` index 5; the lane's `seatOf` pair): `vibes ▾` toggles it — `pool [neighbours | within a tone | any]` · `change [never | rarely | half | often | always]` · `draw [random | exhaust | walk | shadow]` · `seed` (its own box, ENTER = go; the last five as chips, the strip's idiom) · the tolerance read from `mayTake` and shown (`±5¢`) · `go` · the take named as the strip names it, or *pick one* · the choices remembered in the browser (`localStorage`, as `1p`'s switch). **The status** (`H.say`): *`vibes · take "Just-B1-seed193" · pool neighbours (4) · change half · draw exhaust · seed 17 → 12 of 19 breaths changed · seat 1: B5 → C6 → B5 … · seat 2: C♯5 → F♯5 … · 2 empty pools`*. `back` is the existing button (it counts these notes with the rest). The note card's line (`1q.6`) reads the stamp as it does. `composer.html` one script tag.
  - **1u.4 — The SPACE fault in the sequence drawer** (his, LG-114: *"when I select a take for one of the containers the space bar then reverts to starting the main score play"*): READ IN THE CODE — the box's take menu is appended to `document.body` (`sequence_ui.js` `openTakeMenu`, 1405); the drawer's `pointerdown` capture (578) sets `active = this.el.contains(target)`, so a click in the menu is OUTSIDE and the drawer stands down; the composer's own handler (`composer.html` 2411) then plays the score. **The fix, one line:** `inside = this.el.contains(t) || (this._takeMenu && this._takeMenu.contains(t))`. The harmony strip's use of the same menu (`opts`) is untouched — it is not a drawer. **Required check** on `score-5401`: the drawer open → a box's take picked from the menu → a dispatched SPACE → the drawer's `hear` runs and the composer's `togglePlay` does not (the engine stubbed, STILL BINDING); ESC and the text fields as before.
  - **1u.5 — The shield and the required verification** on `score-5401` with the stubs of STILL BINDING (journal §2), every ONE captured: (a) THE SHIELD — the §317 · §319 captures byte-identical (`take` · `shuffle` · `back` on `pointilistic01a`); every stored render untouched by construction (no engine file changes) — `model_bank --validate` · `sequence_check` 180 · `test_snapshots` · `sequence_notation_check` 58 green before and after · (b) `vibes_pitch_check` green · (c) on a READ-ONLY COPY of one of his sequence passages with both vibraphones (`piece-LGMF-Sec01-Sec02`): the marquee → `go` at `always · any · exhaust` → every breath after the first of each seat re-pitched, no two seats on one pitch at any overlap, every pitch a series member under ±5 ¢ in range, onsets · lengths · `velAbs` · `cc7Abs` · nodes byte-identical to before → `back` → the passage byte-identical to before → the same seed → the same result · (d) `half · neighbours · walk` → the changed breaths ≈ half, every step one pool neighbour · (e) a placed morph's vibraphones (`ACT-BLOOM-0x` in his `Sec02` save) the same way — the take resolved from the actual · (f) the captured MIDI of one re-pitched breath: the tempered note on its curve channel, no bend, the velocity and fader unchanged (DYNAMICS_LAW §3; MAIN ch 1 empty) · (g) 1u.4's check.
  - **1u.6 — His one test:** RELOAD the tab (page files only, no restart). A sequence passage with the vibraphones → CTRL+drag the lane → `vibes ▾` → `any · always · exhaust` · `go` → play → `back` → `neighbours · half · walk` · a new seed → keep one. Then the drawer: a box's take from the menu → SPACE. Revise on his word — a design question on Fable, a fault on Opus.
  - **THE AI'S CALLS, his to reverse:** the first breath of each seat kept · the seats by overlap · the others' partial by nearest-within-20 ¢ when unstamped · `exhaust` per seat · the choices remembered in the browser · a new file rather than growing `harmony_sel.js`.
  - **THE BUILD ORDER:** 1u.1 → 1u.5 on Opus after a checkpoint + clear, one commit per step, THE SHIELD in each, RUNNING_LOG § per step; 1u.6 his.

## 2. Notate — `doing` *(2a built 2026-09-25, the names in; 2c THE PAGE EDGES built 2026-09-25, his eye deferred to the notation; 2d THE EH'S SEQUENCE NOTATION PROTOTYPE planned 2026-09-25 — the fixed scale, the just marks — Opus builds next; then 2a.6 the clefs; 2b to be laid out)*
...
- **2d — THE ENGLISH HORN'S SEQUENCE NOTATION: THE PROTOTYPE — one of each thing, on the fixed scale, with the just marks** — `built` 2026-09-25 (Opus, 2d.1 … 2d.6 — RUNNING_LOG §382 … §387; 2d.7 his eye) · `planned` 2026-09-25 (Fable, session 15, at his word *"Okay, go"*; RUNNING_LOG §368 … §380 — §368 the stats · §369 · §370 his sketch and his decisions · §371 the layout and the audit · §372 … §374 the scale · §375 · §376 one of each thing · §377 … §380 the pitch marks; COMPOSITION_NOTES LG-111 · LG-112; `docs/research/just_partials_notation.md` §1a) —
  **Result when done:** a notation page of its own, `notation/ir/lgmf-eh-proto.ir.json` (in the picker AFTER `piece-lgmf`, which is untouched), extracted from a COPY of the newest save, showing the english horn's first three systems (0 … 36 s at 12 s a system) with ONE OF EACH THING: **the block** at 0 s (a full-size open head G♯5 with ¾♯ · its column `+41` over `26°/C1` · "senza vib." above · `pp → mp` on the dynamic row) · **the level curve** in the bottom half-lane on the FIXED SCALE (niente at the floor, fff at the top), green, continuous through the breath gaps, the 6 s fade from nothing drawn by the curve itself, the follower riding it · **the pie** at the lane top counting each breath down · **a breath with the same pitch** at 13.7 s (a go line, a cue-size parenthesised open head to its left, no column) · **a breath with a new pitch** at 28.7 s (a go line, a full-size open head G5 to its left with `+2` over `12°/C2`) · **two labels** on the dynamic row, `(mp)` at 25.0 s and `(pp)` at 30.9 s. THE SHIELD: every existing page byte-identical. Nothing of the morph, no other player, no cents on a reminder head — his scope (§376).
  - **The design line:** a SEQUENCE is a device of the notation like the surge and the morph — `devices.byEnv.sequence` — and it is drawn PER PART: the notes of one part inside a `grp-seq-*` group (`srcKind: 'sequence'`) are one LINE with breaths. The IR carries, per part: the entry (the block's pitch and marks, the written range in words, the technique text) · the level at 100 samples/s as the WRITTEN LEVEL 0 … 1 on the fixed scale (§2d.1) · the breaths (onset, release, `pitch: same | new`, the pitch's marks) · the labels (the turning points, named). The engine draws what the IR says; nothing is computed twice (D9).
  - **THE FIXED SCALE (his decision, §374 · §375):** the half-lane's height is the written level: **eight equal steps from niente (0) to fff (1) — ppp at 1/8 … mp at 4/8 … fff at 8/8**, and between two names linear in CC7 through the instrument's own ladder (`DynTable.cc7`, the inverse of it), under ppp linear to CC7 0 = niente. One scale for the whole piece, every player, every realization; the presentation score's 14 px for pp → mp accepted; the per-block legend is the FALLBACK for the presentation realization only, if his eye says so at 2d.7 [call: the eighths].
  - **2d.1 — The device and the IR:** (a) `notation/registry/devices` gets `byEnv.sequence` (the extractor's env classification in `notation/lib/extract_core.js`, where `surge` · `trill` · the morph are classified; a note whose group is `grp-seq-*` / `srcKind 'sequence'` → env `sequence`); the IR schema amendment for the device's fields. (b) The extraction of a part's line: the notes sorted by onset; **the level** sampled at 100/s from each note's drawn shape through `heldCc7`'s rule (`cc7Abs {lo, hi}` on the nodes' y/10, `cc7Fade` multiplied in — `composer.html` 9758 … 9770; `sonify_core.evalWaveCurve` is the sampler the surge uses), mapped CC7 → written level by the part's ladder; **through a breath gap (0.4 … 1 s) a linear bridge** from the last sample to the next note's first [call]. (c) **The breaths:** every note after the first is a breath — `same` when its `sonifyNote` and cents equal the previous note's, else `new`; the pitch's marks from the SAVE: the partial and the fundamental from the sequence's recipe (`databases.sequences[].recipe.containers[].chord[]` by `lane` · `seat` · box — the box index is in the note's `performanceNotes`, `box N`), the cents from `morphBend` (the same number as the recipe's `cents`). (d) **The labels:** the turning points of the sampled level (a crest or a trough), each named by the NEAREST written name on the ladder; a turning point within 1 s of the previous label carrying the same name is dropped; a flat note yields none [call: the 1 s dwell]. (e) **The entry:** the first note's pitch and marks, the written range in words from the recipe's waves `low` · `high` (`pp` · `mp`), the technique text from the technique (`senza_vel` → "senza vib."). (f) **The glyphs:** the six arrowed accidentals (Emmentaler `accidentals.sharp.arrowup` · `.arrowdown` · `flat.` · `natural.`) baked into `notation/lib/glyphs.json` `accidental` from LilyPond 2.24.4 `emmentaler-20.otf` by the parens' recipe (their `_provenance` names it), with `wSs · hSs · anchors`; and `text["senza vib."]` on the `pizz.` recipe (opentype.js, CrimsonPro-LightItalic.ttf at 1.0998 ss, the `_provenance` of `text`). (g) `node tools/notate_section.js --score <the copy> --sequence grp-seq-smu90t537 --part 0 --range 0-40 --id lgmf-eh-proto` (the flags Opus's — `--morph <group>` is the model) → `ir_validate` → `index.json` with the proto AFTER `piece-lgmf`. The copy: `scores/piece-LGMF-Sec01-Sec02-Sec3start.json` copied to the scratchpad (section 1 is identical in every save, §370) — HIS file never opened for writing. **Required check:** in node, the IR's level at 0 s = 0 · at 6.0 s = pp's height 2/8 (±1 CC7 step) · its maximum in 0 … 36 s at 25.0 s ≈ mp's height (the note's `hi` 69 against the table's 68, §368) · at 30.9 s pp · the breaths at 13.71 `same` and 28.71 `new` with `+2` · `12°/C2` · the labels exactly `(mp)` 25.0 · `(pp)` 30.9 · the entry `+41` · `26°/C1` · `pp → mp` · "senza vib." · the nine glyphs present with sane boxes.
  - **2d.2 — The block:** the header overlay's SEQUENCE variant (`notation/lib/morph_overlays.js` `header` → `layout.js`'s D45 header branch, ~792 … 835, is the model): at the part's entry, in ss offsets from the first go line (never stretching with the zoom) — **one full-size open head** (`notehead.open` scale 1) with its accidental by the bands (§1a of the standard: ¾♯ here) and ledgers · **the column** centred on the head: the cents `text` item at D45's height (`max(y + 0.5, 2) + 0.6` ss, `TS.instruction` 0.75, upright, the true minus, `+41`), the partial one row (1.0 ss) above it (`26°/C1`); a column wider than the head is right-aligned to the head's right edge, never past the go line · **"senza vib."** 0.45 ss above the column's top, from the head's left edge (the `pizz.` rule, `textAbove`) · **`pp → mp`** on `dynY` (−4.6 ss; the 0.45 spacer under lower ink, #5 §479): right to left `mp · 0.45 · arrow(2.0) · 0.45 · pp`, ending one spacer before the go line — the tuba header's chain with the two names in place of circle and mark. No niente sign, no hairpin (§376 (a)). **Required check:** the DOM on `score-5401` (the notation app, `lgmf-eh-proto`, the video view at 0 s): the head at G♯5's y with the ¾♯ · the two texts at their rows, centred (or right-aligned) on the head · "senza vib." 0.45 ss above the partial's ink · the dynamic chain on `dynY` ending 0.45 ss before the go line.
  - **2d.3 — The curve and the follower:** the level curve in the BOTTOM half of the lane (the morph's split — `morph_overlays.js` `fit('level')` is the model; the top half left empty here, the pitch half of the morph to come), D42's closed path at 100/s (`curvePathD42`, CURVE_LOOK §2), limeGreen `#99FF00`, filled, no border, **absolute on the fixed scale — no normalisation, no floor** (D46 · D47's letter, this piece's scale); the fade from nothing is IN the samples (2d.1 b). `animated.crescMeter` rides the samples (`notation/lib/animobj.js`) — exists. **The edge registry:** `page_rules.edge` gets the curve's kind (`screen: cut` like the morph's curve, `print` the same) — the 2c rule: a kind with no entry fails `check_screen_edges`; a row in `docs/NOTATION_STANDARDS.md` §5. **Required check:** the DOM: the curve's y at x(0) = the half-lane floor · at x(6) = 2/8 of the half-lane above it · at x(25.0) ≈ 4/8; `node tools/check_screen_edges.js --ir lgmf-eh-proto` exit 0.
  - **2d.4 — The breaths:** at every breath a **go line** (the morph's "one go line per breath", `morph_overlays.js`; the reading regime of NOTATION_STANDARDS §1: head before the line) and **the head to its LEFT** — the tuba's `onsetHead` / `onsetAcc` device flags (built, unused — MORPH_NOTATION "One go line at every breath onset") revived for the sequence device: `same` → a **cue-size (0.844) parenthesised open head** (`leftParen` · `rightParen`, the trill neighbour's mechanism), no column, no accidental change; `new` → a **full-size open head** with its accidental by the bands and its column (2d.2's rule; here `+2` over `12°/C2`, a plain head). The head's right edge one `nhGapSs` (0.25) before the go line. **The pie:** `animated.motivePie` → `enabled: true` in this piece's registry with its numbers (topPx 14 · radiusPx 9 · `#607D8B` @ 0.75 — CURVE_LOOK §7's box: side staffHeight/4 at the lane top, 2 px left of the meter), re-pointed from "a score group" to **the device's breaths**: progress = (t − onset) / (release − onset), the remaining sector filled, full at the onset, empty at the release, the hand black 1 px; between a release and the next onset nothing [call: the registry's colour, the AI's]. **Required check:** the DOM at x(13.71): a go line, a parenthesised head 0.844 to its left at G♯5, no texts · at x(28.71): a go line, a full head at G5 with `+2` · `12°/C2` · with `requestAnimationFrame` replaced by the 16 ms timer (journal §2 STILL BINDING), at t = 20 s the pie's sector = 1 − (20 − 13.71)/(27.97 − 13.71) ± 0.02, at t = 28.3 s (the gap) no pie.
  - **2d.5 — The labels:** a `(dyn)` at each of the IR's labels — the dynamic glyph at the cue scale (0.75 of `TS.dynamic` 0.9) between `leftParen` · `rightParen`, on `dynY`, centred on x(t) — a new drawn kind: its `page_rules.edge` entry (`screen: clamp` — a stamp, never a go-time indicator — `print` the same) and its §5 row. **Required check:** the DOM: `(mp)` centred on x(25.0), `(pp)` on x(30.9), both on `dynY`, nothing else on the row in 0 … 36 s but the block's chain.
  - **2d.6 — THE SHIELD and the required verification:** THE SHIELD BY DATA ABSENCE — no `sequence` env in any existing IR, the pie draws only for a device with breaths: (1) the eight engine batteries (render · layout · animobj · splice · graphic · pattern_fit · stamps · ir_validate_battery) GREEN before and after; (2) 2a's recipe — the tuba goldens staged from #4's HEAD (excluding `index.json` and the README), `export_video --ir db1 --view video --probe 5,11.9,100,300` and `--view zoom --probe 100`, `export_print --ir db1 --planJson` and `--htmlOnly --pages 1-3` BYTE-IDENTICAL, HEAD's engine vs the build's, both on the registry of the commit BEFORE 2d.1 (§344's rule); (3) `piece-lgmf` re-rendered on the build: `export_video --ir piece-lgmf --view video --probe 300` byte-identical to HEAD's (the registry's pie switch must not touch it); (4) `check_screen_edges` on BOTH `piece-lgmf` and `lgmf-eh-proto`, exit 0; `check_print_edges --ir lgmf-eh-proto`; (5) every REQUIRED CHECK of 2d.1 … 2d.5 run and its numbers in the RUNNING_LOG; (6) ONE screenshot of the video view at 20 s — the proof for his eye, the only screenshot. One commit per step, pushed; a RUNNING_LOG § per step (read the last heading first — §381 is the next free at this writing).
  - **2d.7 — His eye:** the notation app (no restart — page files and the IR) → the picker → `lgmf-eh-proto` → the video view, `0` in the time box → the block; play or type `20` → the curve, the follower, the pie mid-breath; `28.7` → the new pitch; `z` the zoom. **What it settles, in his order:** the scale on the eye (the 14 px swing — keep, or the per-block fallback for the presentation realization) · the rows' order under the staff · the column's look and the arrow glyphs (none in 0 … 36 s — the first is at 142 s; `--range 140-160` shows one) · the pie · 12 s a system · whether the opening sign returns from the performance notes. Revise on his word: a look question on Fable, a fault on Opus.
  - **FOUND AT THE BUILD — THE PAGE-START CLASH, for 2d.7 (RUNNING_LOG §383 · §387):** the EH enters at 0 s = t0 of page 1 and the block has no room left of x(0) — on SCREEN the 2c clamp moves it right of its own go line; on PAPER it is drawn into the clef gutter and "senza vib." straddles the gutter's edge (`check_print_edges --ir lgmf-eh-proto` fails page 1; every other gate passes). His, one decision for both media: (a) a LEAD-IN — the piece's 0 a few seconds into the first page (the screen tiling offset; paper's page-1 t0 placed by the objects, as a pushed GC's is) [the AI's recommendation] · (b) the clamp on both · (c) as they are.
  - **Deferred at his word (§370 · §380):** the morph's block and the join at 149 → 158.4 s (what carries over) · the other six players and the percussion · the strings' `°` collision (partial vs harmonic circle) · the legend sentence and the performance notes · the closing sign.
  - **The AI's calls in this item, his to reverse:** the eighths of the fixed scale · the linear bridge through a breath gap · the 1 s dwell on the labels · the cue scale 0.844 for the reminder head, 0.75 for a `(dyn)` · the pie's registry colour · `n°/F` always written on a part's line · the proto's range 0 … 40 s extracted, three systems shown.
  - **Build on OPUS from this item, 2d.1 → 2d.6 in order, one commit each; STOP after 2d.6 for 2d.7.** A checkpoint and a clear first.
