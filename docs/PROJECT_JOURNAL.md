...
|---|---|---|---|
| **► 1b.5 REMEASURE** | **1b.5 RAN AND FAILED — as it was written to be able to** (RUNNING_LOG §88, `bank/verify_1b5.json`). First run through the app’s own path; **the mechanism is right** (the capture shows the vibraphone’s CC7 118/75 exactly as the bank prescribes) and **the true peak passes at −5.0 dBTP**. What is wrong is what the card BELIEVES: per-part spread **18.8 dB** (wanted ≤3), tutti fff **−14.3 LUFS** (wanted −20±2). **Two faults, both of measurement resolution:** (1) the vibraphone’s register was sampled every 4–5 semitones and the zones are finer — **pitch 74 is 11.5 dB above both its neighbours 71 and 76**, and most of the piece’s vibraphone pitches fall between samples; (2) the curves are **single strikes** on samplers scattering 2.2–3.2 dB SD — the cello’s pitch 60 reads LOUDER at velocity 100 than at 127, skewing the inversion 7.1 dB. **NEXT: re-measure with resolution** — every semitone on the vibraphone (~5 min, deterministic now) and three repeats per velocity point on the four Kontakt instruments (~10 min) — then rebuild the remap and re-run 1b.5. The SI2 three land within 1.5 dB and need nothing | Opus | no |
