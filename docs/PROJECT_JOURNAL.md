...
|---|---|---|---|
| **► 1b.5** | **1b.4 IS DONE** (RUNNING_LOG §86): `bank/velocity_remap.json` rebuilt from the card, **written span ~10 dB → 17** (his option C), scale re-indexed to the app’s real 65–127, every curve normalised per instrument. **Read back through `velocity_remap.js` itself: six of seven instruments within 0.3 dB of target at pp · mp · mf · f · fff.** The vibraphone was measured properly first (9 pitches × 4 velocities on its curve channel) and its loud bars now land exactly on target; its quiet bars are flat at mf but 6–9 dB short at fff — arithmetic, not a bug: 29 dB per bar minus a 15.4 dB register spread leaves 13.7 dB common, under the 17 dB span. **NEXT 1b.5:** the reference chord at pp · mf · ff · fff through the app’s own path, recorded on REC — pass stated in advance: every part within ±1 dB of −31.84 at fff, spread ≤ 3 dB, tutti −20 LUFS-S under −1 dBTP. **Then his ear.** **One question waiting for him there:** whether to enable the per-register **CC7** trim to close the vibraphone’s top 6–9 dB — the app implements it, 0d measured the data, but it reverses part of D13 | Opus | no |
