...
| ✓ | **`1q.3` THE STACK — BUILT AND VERIFIED 2026-09-24** (Fable, RUNNING_LOG §314): the existing cycling (`pickFromStack`, 2026-09-09) verified with REAL clicks on a three-stack built in the throwaway (1/3 → 2/3 → 3/3 → 1/3, nothing moved) · ALT+click = the LIST on every player lane (`HarmonySel.openStackPicker`; a real ALT+click opened it, a row chosen by a real click; META keeps its own picker) · the place in the stack on the strip (`2 of 3 in this stack — click again for the next, ALT+click for the list`) · a TRILL zone in `stackAt`, named `trill C3 +2`; a press on the selected trill cycles, a trill picked by a note's press is selected, never dragged as a note | Fable | — |
| ✓ | **`1q.5` THE TAKE NAMED · `1q.6` THE NOTE CARD · `1q.7` THE SHUFFLE — BUILT AND VERIFIED 2026-09-24** (Fable, RUNNING_LOG §317; planned the same day from his `1q.4` feedback, §316 · LG-104 · PLAN § `1q.5` … `1q.8`): the strip names the take a selection came from, read from the note (`hq.take` · `midi` · `cents` · `partial` · `seed` · `dealt`, or the `performanceNotes` fragment of a note written before — one parser `HarmonySel.info`), the menu's blue name the selection's own · the card's line `F5 +5¢ · partial 17 · ← take "…"`, an octave keeps the cents, the partial stands, `(moved N st)` off-octave · `shuffle` · `seed` · five chips on the strip — each selected note ANOTHER partial of ITS take's series (the take loaded as `dealTake` loads it, `D.voices`), the player's range for the note's own technique, the take's `mayFold`, `D.mayTake`; a seed a pure function of the selection (found and fixed: the exclusion against `hq.dealt`, not the current partial). Real clicks on `+8va` · `shuffle` · a chip; all 254 objects identical at the end, 0 POSTs; `strike_drawer.js` untouched | Fable | — |
| ✓ | **THE VOLUME FIX — BUILT AND VERIFIED BY CAPTURE 2026-09-24** (Fable, RUNNING_LOG §318 the diagnosis · §319 the build; DYNAMICS_LAW §3 **Rule 5**): his ear — *"if I apply a take like just-b1-seed208 it seems louder"* — a take with cents un-plained a STRUCK note and the drawn anchor scale's floor (65) lifted his velocity 45 to ≈ 87. Now `writeNote` pins `velAbs = recVel` · `cc7Abs {127,127}` when it un-plains a struck note; `back` restores the pin; the shuffle the same. Captured on his `pointilistic01a`: `lghorn ch1 note 69 vel 45` → after the take `lghornb ch4 note 75 vel 45` with its bend, CC7 127 → `back` byte-identical | Fable | — |
| ✓ | **`1q.9` THE DYN BOX — BUILT AND VERIFIED BY CAPTURE 2026-09-24** (Fable, RUNNING_LOG §320; LG-105; his *"am i able to multiselect and batch update the dynamic?"* → *"go"*): `dyn ▾` and a height box on the strip set every selected note through `NoteCard.applyLevel`, the card's setter lifted out and put under the law — struck by velocity (§318's pin following), full-fader by height, shaped by the table (the card too, now), a trill skipped; one undo; the boxes read the common dynamic back. Captured: five notes to `ff` by real form input → the played four at vel 109, the shaped one on the table's `ff` (100, after a rounding fix); the card's own box on one note unchanged; 0 POSTs. Seen, not touched: 17 notes in his score in the DRAWERS' bent-insert shape (recVel, no plain, no velAbs — §318's fault by the drawers' own door), his to raise | Fable | — |
| **►►** | **`1q.8` HIS ONE TEST — RELOAD the tab (page files only).** Also the dyn box: a marquee over a passage → `dyn ▾` → `ff` → play; CTRL+Z. First his ear on the level: the same passage (`pointilistic01a` 15.64 s), `Just-b1-seed208` onto it — no louder than his playing. In his score: one re-pitched note selected → the strip names its take, the menu lights it → P → the card's line (note · cents · partial · take), `+8va` keeps the cents → a chord passage where one part has several notes → `take ▾` → `shuffle` (the three of one part on different partials), `shuffle` again, a chip back, `back`. Revise on his word | his ear · then Fable | — |
| ✓ | **`1q.4` HIS ONE TEST — came back 2026-09-24 as feedback, RUNNING_LOG §316 · LG-104** (the take's name on the strip and the card · the cents shown · an octave and the cents · a shuffle) → planned and built the same day as `1q.5` … `1q.7` above. *(Was:)* RELOAD the tab (page files only). In his score: a CTRL+drag over a chord passage (the real CTRL+drag — the pane could not hold CTRL, §313) → the strip at the top right counts the notes → `take ▾` → `▸` a take (heard, not chosen) → its name → the notes re-pitched (the status names them), play the score → `back` → a second take on the same selection · SHIFT+CTRL+drag adds · a chord on one part: click, click, click (the strip counts the place), ALT+click for the list · a trill in a selection. Revise on his word | his ear · then Fable | — |
...
- **WHAT TODAY PUT IN HIS HANDS (all committed, pushed):**
  - **`1q` (PLAN § `1q`; LG-103; RUNNING_LOG §306 … §311 the talk and the plan, §312 · §313 · §314 the builds; commits `f44d4f1` plan ·
    `aa89d13` · `376b9b4` · `431a1af`):** `score/public/harmony_sel.js` (new, a mixin) — THE HARMONY STRIP at the top right of the score
    whenever pitched notes or trills are selected (`harmony · N notes · M players · take ▾ · back (n)`); `take ▾` is the sequence drawer's
    takes menu through a new optional `opts { onChoose, current }` on `SequenceDrawer.openTakeMenu` (`sequence_ui.js`, five lines); a chosen
    take re-pitches every selected note from ITS player's note of the take (cents as `morphBend`, the note DRAWN; the vibraphone's two seats in
    time order; a trill its lower note, interval kept, no cents); the original remembered ONCE on the object (`hq.was`), `back` restores it ·
    **CTRL+drag** on empty lane space = the marquee (touched objects on player lanes; SHIFT+CTRL adds; ESC cancels) — one block in the container's
    mousedown, BEFORE the SHIFT-span branch · **the stack:** the 2026-09-09 cycling (`pickFromStack`) verified with real clicks, **ALT+click**
    the list on every player lane, the place on the strip (`2 of 3 in this stack`), a trill zone now in `stackAt`.
  - **The top bar (RUNNING_LOG §315, commit `bd9f707`)** — his *"move that passages menu over … because I can't select the solo for English
    horn"*: the fixed 32 px bar wrapped and hung over the first lane. The literal move and a right-align were tried and REJECTED (fragile:
    at 1000 px a full wrapped line still covered the solo). Kept: `#topBar` grows (`min-height`), the lanes, the save hints and the
    Insertion strip start at `var(--barH)`, set by `Composer.fitTopBar()` (ResizeObserver + resize); `#saveStatus` one fixed share
    (`flex 100 1 36em`, cut with …, whole text on hover) so a status never moves the lanes. `passages.js` unchanged. The cost: the lanes shorter
    by what the bar wraps.
- **THE NEXT CONCRETE STEP IS HIS. Tell him, then wait:** reload the tab → `1q.4` (journal §2 ►► row): a REAL CTRL+drag over a chord passage
  (the pane's drag tool could not hold CTRL — that path ran on dispatched events) → the strip counts → `take ▾` → `▸` a take → its name →
  play → `back` → a second take · SHIFT+CTRL+drag adds · a chord on one part clicked three times, ALT+click · a trill in a selection · the
  English horn's solo now clickable at his window width. Then `1p` · `1n.6` · `1o.6` as checkpoints #8 · #7 say. Revise on his word, on Opus.
- **`Resume reads:`** only for his feedback — on `1q`: RUNNING_LOG §312 · §313 · §314 (the step he faults) and PLAN § `1q`; on the top bar:
  §315; on `1p` · `1n` · `1o`: as checkpoints #8 · #7 name. Journal §2 STILL BINDING before any verification. Code: `score/public/harmony_sel.js`
  first. **Nothing else.**
- **Decisions pending him (new today):** the calls of §312 (the strip at the top right · the key `hq` on the object · a note bent before and
  straight now on the RECENTRE bend · a seat's note DRAWN · the `performanceNotes` stamp · the time-order round robin for the vibraphone · the
  selection kept after a take · `opts` on the drawer's menu rather than a clone) · §313 (touched, not enclosed · META and the curve windows out)
  · §314 (the list's look · the place on the strip) · §315 (the lanes shorter when the bar wraps · the status cut at 36em). **Carried from #8:**
  a column `dyn` box · the bottom bar scrolling sideways · a brick dragged to another lane keeping its articulation · the calls of §303 · §304 ·
  his word on `1p` · `1n.6` · `1o.6` · `1m.4` (§284 · §285) · whether `bank/patterns.json` goes into git · the triangle on track 10 (§221) ·
...
- **DELIBERATELY UNCOMMITTED — all his, none of it mine to touch** (`git status --short`): `bank/panel_snapshots.json` (his harmony takes,
  ...
  `bank/rhythm_takes.json` · `bank/rhythm_sequences.json` (the Texture stores of `1l`) · untracked `bank/patterns.json` (his patterns; git
  is his call, §278) · untracked `scores/pointilistic01a.json` (HIS score, saved by his tab; no score is committed here). The throwaway
  `score-5401` wrote nothing (every POST caught, 0 in every run; its three localStorage keys cleared; stopped). **Unsaved working copies**
  (`node tools/unsaved_check.js`): `cresTest` · `lgmf-all` · `lgmf-bloom` · `longToneTest` — the same four as #2 … #8, none the piece — and
  **`pointilistic01a`**, his score, the working copy a minute newer than the file: he is working in it; his to Save.
