...
### SESSION 16 · IN PROGRESS (2026-09-26, Fable) — HIS NEW SUBJECT AT THE `/postclear`: `1u` THE VIBRAPHONES' PITCHES ON THE STRIP — PLANNED IN FULL, NOT BUILT (PLAN § `1u`; RUNNING_LOG §388 · §389; LG-114). The cold-start block is the ►► `1u` row of the table below + PLAN § `1u`. **`Resume reads:` for the build:** PLAN § `1u` whole · RUNNING_LOG §389 (what was read in the code) · `score/public/harmony_sel.js` (the strip: `pool` · `range` · `remember` · `writeNote` · `stamp` · `apply`) · `sequence_ui.js` 570 … 600 (the SPACE fault) — nothing else until a step names it. Session 15's checkpoint #7 (2d.7 · the page-start call) stands below under *Open at session end*. **Working title: _Recombination_ (D32).**
