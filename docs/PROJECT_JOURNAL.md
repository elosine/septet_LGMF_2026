...
| ► | **BUILD `1q.2` THE MARQUEE** (PLAN § `1q.2`): CTRL+drag on empty lane space — one line in the container's mousedown, `HarmonySel.beginMarquee`; touched bricks on player lanes selected as SHIFT+click builds it, SHIFT adds, the after-click swallowed, ESC cancels; its REQUIRED VERIFICATION with the pane's real input → then `1q.3` THE STACK (verify the existing cycling · the ALT+click list on every lane · the place on the strip · the trill's press) → `1q.4` his one test (reload the tab) | Fable, at his word | — |
