...
| **►►** | **BUILD `1n.2` THE SINGLE NOTE BY HAND** (PLAN § `1n.2`): a `dyn` box per row of the players list in texture mode (one, two or three names — `mp` · `mp-f` · `mp-f-mp`, `n` at an end; refused with the reason; on a short note the first name), the lens writing it to every selected column, "mixed" where they differ, the column's MARK beside each lit circle (a letter-mark, a wedge for a hairpin — [call on the look]), undo; its REQUIRED VERIFICATION with the pane's REAL input and THE SHIELD on `score-5401` — then 1n.3 → 1n.4 → 1n.5, one commit each; 1n.6 his | Fable (his word: *"build as much as possible independantly"*) | — |
...
**Open at session end — CHECKPOINT #7 (2026-09-22, session 13, Fable — THE `1n` BUILD IN PROGRESS) *(mid-session checkpoint — READ THIS ONE)*:**
...
- **THE NEXT CONCRETE STEP, as an instruction: build `1n.2` the single note by hand** (the ►► row above; PLAN § `1n.2`), then 1n.3 · 1n.4 · 1n.5, one commit each, each step's REQUIRED VERIFICATION and THE SHIELD on `score-5401`; `1n.6` his. The pattern document gains per column `dyn` (a typed string, or a generator's `{ pts }`), `hand`, `fol`; the doc `ranges` at 1n.3 — the undo snapshot must grow with them.
