...
|---|---|---|---|
| **► 1b.3** | **The vibraphone is FIXED** (RUNNING_LOG §82). He made **preset 13 "Bowed Velocity RRoff"** — round robin off, slot rr off — in all four instances; the recipe’s `bowed_vel` keeps its key and points at it (`cc0: 12`), so **all 641 vibraphone notes already written inherit the fix untouched**. Measured: the worst repeat spread fell **13.8 dB → 0.0**, every reading exactly the value predicted before the run. The register was then re-measured clean (47 stale rows purged): still **15.5 dB** across the range, but **the two velocities agree on every step within 0.3 dB** and the velocity response is uniform (13.5–14.0 dB at all nine pitches) — so it is **one constant offset per pitch, velocity-independent**, which the velocity remap can carry with no new mechanism. **NOW 1b.3:** the absolute target (nine voices at fff → −20 LUFS-S, ≤ −1 dBTP), trims for the fourteen percussion and the seven pitched, the vibraphone’s per-pitch offsets, sustained families judged on the INTEGRATED figure | Opus | no |
