...
- **Decisions pending him:** **the SI2 brass carry a curve copy for `ord` ALONE** (`curveTechniques: ["ord"]` on the `b` ports) — a brass
  short on `staccato` (Texture's own default of `1l.1`) therefore stays on velocity alone, off the one scale; the status says so; a second
  voice on the `b` instances is a rack question (§296) · the calls made alone in §296 … §300 (the flag on attacks alone · the vibraphone's
