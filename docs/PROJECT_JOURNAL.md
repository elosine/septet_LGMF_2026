...
|---|---|---|---|
| **► 1b.3** | **1b.1 and 1b.2 are DONE** (RUNNING_LOG §77, §78). The chain is unity and the meter proven (bank/reference.json); **bank/instrument_card.json** holds all seven instruments at four velocities on three pitches, in absolute dBFS at the master, measured two ways. **Next: 1b.3, the trims to an ABSOLUTE target** — nine voices at fff → −20 LUFS-S and ≤ −1 dBTP, so one voice at fff ≈ −30 LUFS-S; sustained families judged on the INTEGRATED figure, one-shots on max-momentary. **Two things 1b.3 has to settle with him:** the **vibraphone’s 16.3 dB register spread** — a per-instrument trim cannot express it, so per-register treatment or the writing avoids the loud band — and whether to **re-measure all fourteen percussion** rather than carry 0d’s over (the triangles and castanets miss by ~3 dB; NITS). Lay out the sub-steps with him first | Opus | no |
