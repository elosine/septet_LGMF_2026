...
| **►►** | **BUILD `1o.2 starting and recalling`** (PLAN § `1o`): `texture ▾` starts a new empty pattern, `pattern ▾` recalls one by its own name, several per texture, keyed by `id` in the browser; its REQUIRED VERIFICATION and THE SHIELD on `score-5401`, one commit → then 1o.3 (**restart the server after it**) → 1o.4 → 1o.5 → 1o.6 his → then `1n dynamics` | Opus or Fable | at a step boundary if the context is long |
...
- **`1o.1 the document` IS BUILT (2026-09-22, Fable, RUNNING_LOG §291) — the next concrete step is now `1o.2 starting and recalling`, the instruction below otherwise unchanged (its verification method, its order, its commits).**
