...
|---|---|---|---|
| **► HIS CTRL+S, then 1b.5** | **1b.4 IS COMPLETE, vibraphone included** (RUNNING_LOG §86, §87). The remap is rebuilt from the card at a **17 dB written span** (was ~10) and six of seven instruments land within 0.3 dB of target at every height. **The vibraphone’s register is now carried by CC7, at his word** — confirmed first that no bank had ever used `cc7Curve` and no score uses `cc7Abs`, so it touches the vibraphone alone. Its fader moved **−6.62 → +2.81** (referenced to its QUIETEST bar, because CC7 can only attenuate), one shared table carries the dynamic, CC7 carries the 15.4 dB spread: **every used pitch within 0.2 dB at pp, mf and fff, no clamps.** **HIS: CTRL+S** — the vibraphone fader is unsaved. **THEN 1b.5:** the reference chord at pp · mf · ff · fff through the app’s own path onto REC; pass stated in advance — every part within ±1 dB of −31.84 at fff, spread ≤ 3 dB, tutti −20 LUFS-S under −1 dBTP. Then his ear | Opus | no |
