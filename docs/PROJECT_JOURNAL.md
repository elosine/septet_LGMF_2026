...
- **THE NEXT CONCRETE STEP, as an instruction: build `1n.4` the pointillistic model** (the ►► row above; PLAN § `1n.4`), then 1n.5, one commit each, each step's REQUIRED VERIFICATION and THE SHIELD on `score-5401`; `1n.6` his. The pattern document gains per column `dyn` (a typed string, or a generator's `{ pts }`), `hand`, `fol`; the doc `ranges` at 1n.3 — the undo snapshot must grow with them.
