...
|---|---|---|---|
| **► APPLY 1b.3** | **The trims are COMPUTED** (RUNNING_LOG §83, `bank/trims.json`). Target: nine voices at fff → −20 LUFS-S, so **each voice at −31.84 dB** on the card’s scale; the K→LUFS offset **+2.30 dB** read off the proven tone. **Every instrument comes down, 6.6 to 21.3 dB — not one boost.** Vibraphone fader −6.62 (from +9.30) with its per-pitch residuals held for 1b.4. Headroom: the chord tutti peaks **−9.6 dBFS, 8.6 dB under the ceiling**. **His clipping is located:** sleigh bells **+10.6 dBFS** and tambourines **+0.6** over full scale on ONE note, both from 0d’s relative boosts. **NEXT: apply** — regenerate `apply_trims.lua` from `bank/trims.json`, read every value back, rewrite `balanceDb` in the recipes and `bank/perc_rack.json`, **his CTRL+S**. Then 1b.4 (the remap, carrying the vibraphone’s register) and 1b.5, whose pass is already stated: every instrument within ±1 dB of −31.84 and a fff tutti at −20 LUFS-S under −1 dBTP | Opus | no |
