...
|---|---|---|---|
| **► 1b.1** | **PLAN 1b — the rack calibrated to a standard (PLAN.md § 1, RUNNING_LOG §76; top line approved 'good' 2026-09-19).** Open with 1b.1, the reference in the rack: a `REF` utility track with −20 dBFS RMS pink noise + 1 kHz, master and REC at 0 dB, recorded on REC and read back by the analyzer at −20.0 ± 0.1; then he sets the system volume once on it. Lay out 1b.1's sub-steps with him first (PLANNING_METHOD), then build; 1b.2 → 1b.5 follow in order; 1b.6 gathers them into `tools/qc_rack.js`. The harness is 0d's (`balance_schedule.js` · `make_rec_track.lua` · `analyze_lgmf_balance.py` · `apply_trims.lua` · `build_remap.js`) | Opus | **yes** — a fresh context, then `/postclear` |
