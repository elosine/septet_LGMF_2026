...
| **►** | **HIS TEST OF 1d.10 — the 1e way:** a waved sequence with two ranges, recorded as MIDI in his rack → `node tools/reaper_job.js run reaper/bridge/jobs/cc7_by_channel.lua`. **Expected: two DIFFERENT CC7 spans**, neither topping at 127 unless its `high` is `fff` · MAIN ch 1 empty · every strike an mf velocity. **Then the running order continues: 1d.11** the library (`bank/sequences.json`, a file of its own) → **1d.12** select a range → **1d.13** the waves by preset → **1d.9 + 1d.14** the breath's lengths → **1d.15** the clock and the cursor → **1d.6** his listen. The two questions that were his are ANSWERED (RUNNING_LOG §149): `of max` is ON at 0.65 on a new sequence · `±` is ONE number of seconds for everyone, built-in default 1.3 s, with a `save as default` on the `breath` line. Nothing in the plan waits on him now. | **Opus** — executing a written plan | **yes** — checkpoint, clear, build cold |
...
- **1d.10 IS BUILT (RUNNING_LOG §150) — SO THE NEXT CONCRETE STEP IS `1d.11`, THE LIBRARY.** Open `docs/PLAN.md` § 1d.11 and
  execute it as written: `bank/sequences.json`, a **file of its own** (never a panel in `panel_snapshots.json`, 3.1 MB rewritten
  whole on every save) · a `store` field on `/api/snapshots` checked against a WHITELIST of two names · panels `library` ·
  `untitled` (a rolling stack of 50) · `wavePresets` · autosave to `localStorage` on every change and to DISK ~2 s after the last
  one and on `pagehide` · naming MOVES an untitled entry · `save` / `revert` / a `•` when they differ · a `library` menu with
  `duplicate` and `×`. **His test is written at the foot of the item.**
- **BEHIND IT, HIS: THE 1d.10 RACK TEST** — a waved sequence with two ranges, recorded as MIDI in his rack, read back by
  `node tools/reaper_job.js run reaper/bridge/jobs/cc7_by_channel.lua`. Expected: **two different CC7 spans**, neither topping at
  127 unless its `high` is `fff` · MAIN ch 1 empty · every strike an mf velocity. It does not block 1d.11.
- **`Resume reads:`** *(what the NEXT step needs, nothing more — the history is in RUNNING_LOG and stays there)*
  - `docs/PLAN.md` § **1d.11** — the item itself, which is the instruction.
  - `score/snapshots.js` and `tools/test_snapshots.js` — the merge rules and the battery 1d.11 reuses for the second store.
  - `docs/SEQUENCE_TOOL.md` § **9** (the drawer) and § **10** (the round trip) — where the row lives today, and what `new`,
    `sequences in this score` and Insert already do with it.
