...
|---|---|---|---|
| **► HIS PANEL, then 1b.3** | **He chose A — tame the round robin** (RUNNING_LOG §80). The cycle is characterised: **three members, sequential, resetting at every run start, up to 13.8 dB apart at F♯5.** The documented remote — **CC#82, 21–41 = "Round Robin off"** — is **DEAD on this instrument**: sending it changed nothing, and neither did CC#82 = 123 (Ensemble), which could not be subtle; meanwhile CC#0 preset selection works on the same channel every note. So the cycling is almost certainly the **Slot Round Robin** menu, which the manual gives **no CC for**, and Kontakt’s Lua API reaches the rack, not a KSP panel. **HIS, small:** open **Vibraphone XS**, look at the **Round Robin** and **Slot Round Robin** menus, say what they read, set the cycling off, save. Re-measuring is 2 min (`card2_schedule.js --mode repeat` → `analyze_card.py`). **NOT blocked meanwhile: 1b.3 can compute every trim but the vibraphone’s** — the fourteen percussion are measured and deterministic, and the six have 0d’s repeat-averaged anchors on the absolute scale | Opus | no |
