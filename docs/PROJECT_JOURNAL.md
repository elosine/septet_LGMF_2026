...
- **[SESSION 9: 1d.2 IS BUILT TOO — RUNNING_LOG §115. His test of 1d.2 is outstanding; the next concrete step after it is PLAN 1d.3, the round trip.]** **[SESSION 8: 1d.1 IS BUILT — RUNNING_LOG §114. What follows is the checkpoint as it was written.]** **THE NEXT CONCRETE STEP — build PLAN 1d.1, the generator, exactly as the plan writes it.** `score/public/sequence.js` (pure, UMD,
...
- **`Resume reads:`** *(REWRITTEN in session 9 for the next step — what 1d.3 needs, nothing more)*
  - `docs/PLAN.md` § **1d.3 only** (from `- **1d.3 — The round trip`).
  - `docs/SEQUENCE_TOOL.md` — §9 above all: the drawer as built, the database entry's shape, the one-row-one-place rule 1d.3 grows from.
  - `score/public/sequence_ui.js` — the one file 1d.3 extends (the row, `insert`, `restore`). Read it whole; it is ~330 lines.
