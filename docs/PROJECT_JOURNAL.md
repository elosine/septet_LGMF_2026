...
|---|---|---|---|
| **► HIS, 3 CLICKS, then 1b.3** | **The panel is read** (RUNNING_LOG §81): **Round Robin = "Repetition"**, Slot rr already off. That is the one mode that fires on REPEATED notes — and the vibraphone holds its bars by re-bowing the same pitch ~11×/minute, so the round robin is firing on essentially every vibraphone note in the piece. **Why CC#82 looked dead:** the Round Robin menu is a per-preset "global parameter", and every note re-selects preset 12 with `cc0 = 11` immediately before, so a CC sent at play time is overwritten by the preset's own stored value. **HIS ACTION:** set Round Robin → **"Round Robin off"**, press **Save Preset** (so it survives CC0 re-selection), **CTRL+S** in Reaper. Then a 2-minute re-measure; expected: four identical readings per pitch, settling on 62 → −13.6 · 71 → −20.7 · 76 → −18.0 · 80 → −5.2. **1b.3 is not blocked** — every trim but the vibraphone's can be computed now | Opus | no |
