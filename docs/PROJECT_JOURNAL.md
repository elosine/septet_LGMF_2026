...
|---|---|---|---|
| **► HIS PANEL (3 instruments), then rebuild** | **The remeasure is done and it found more than expected** (RUNNING_LOG §89). **(1)** The vibraphone’s register is **per-BAR, not per-zone** — every semitone measured, spread **19.5 dB**, and 76→77 jumps **14.7 dB in one step**. Now fully measured (37 notes), so the remap needs no pitch interpolation at all. **(2) The cello has the vibraphone’s round robin:** four strikes gave −27.9 · −31.3 · −22.7 · −27.9 — **1st and 4th identical, a deterministic 3-cycle spanning 8.6 dB**. That is exactly why its velocity 100 read louder than its 127 and why it missed by 7.1 dB in 1b.5. English horn (3.6 dB) and double bass (2.9 dB) scatter without a clear cycle in four strikes. **Repeats are NOT the answer** — averaging an 8.6 dB cycle still leaves the instrument lurching between samples in the piece. **HIS: Round Robin → off, all four Kontakt slots each — English Horn preset 12, Cello preset 6, D. Bass preset 6** (`senza_vel` on each). **Edited IN PLACE, not copied**: no recipe change, every existing score keeps working, and the default is recoverable from the .nki. **THEN:** re-measure those three (~4 min), rebuild the card, trims and remap, re-run 1b.5 unchanged | Opus | no |
