...
|---|---|---|---|
| **► HIS CALL, then 1b.3** | **1b.3a done** (RUNNING_LOG §79): all fourteen percussion measured in absolute dBFS — solid, Spitfire scatters under 0.8 dB — and **the vibraphone question is answered differently than expected.** Its 16 dB spread is largely a **three-sample round robin, members up to 13.8 dB apart at one pitch**; 0d had already logged its scatter at **5.99 dB SD**, worst of the ensemble. So a per-register trim would fit the round robin, and no trim can stop a 14 dB jump between consecutive notes. **HIS CALL: tame the round robin in Kontakt (disable it, or level its members), pick another vibraphone preset, or accept it.** Then **1b.3**: the percussion trims can be computed now; for the other six pitched instruments use 0d’s repeat-averaged anchors carried onto the absolute scale (§78 showed `new = 0d + 12 + trim` holds within 0.9 dB) rather than the card’s single strikes, which carry ±2–3 dB | Opus | no |
