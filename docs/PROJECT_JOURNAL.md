...
|---|---|---|---|
| **► 1b.4 (build the remap)** | **The span question is settled, and a real bug was found on the way** (RUNNING_LOG §85). **§56’s UVI Dynamic fix had only ever reached part 1 of the MAIN instance — the piece plays the `b` instances’ curve copies, which were all still at the factory 0.70.** Every horn and trumpet note in all five scores has sounded on ~8 dB of velocity range, not 26. He set all six curve parts to 1.00. Re-measured on the curve channels: **bassoon 31.9 dB (was 11.7), horn 26.4, trumpet 23.5 — and all three read −31.8 at velocity 127 against the 1b.3 target of −31.84, so the trims hold and `Dynamic` extends downward without moving the top.** The Xsample four were checked per pitch: curve = main + trim, residual **0.0 dB on the vibraphone** (deterministic now) and inside scatter on the rest. **The cello is now the narrowest at 17.7 dB, so a ~17 dB written span clears everyone with no clamping.** NEXT: build the remap at that span, normalising per instrument (the card has mixed provenance and says so), carrying the vibraphone’s per-pitch register offsets. Then 1b.5 | Opus | no |
