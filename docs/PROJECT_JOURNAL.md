...
| **►►** | **`1q.4` HIS ONE TEST — RELOAD the tab (page files only).** In his score: a CTRL+drag over a chord passage (the real CTRL+drag — the pane could not hold CTRL, §313) → the strip at the top right counts the notes → `take ▾` → `▸` a take (heard, not chosen) → its name → the notes re-pitched (the status names them), play the score → `back` → a second take on the same selection · SHIFT+CTRL+drag adds · a chord on one part: click, click, click (the strip counts the place), ALT+click for the list · a trill in a selection. Revise on his word | his ear · then Fable | — |
