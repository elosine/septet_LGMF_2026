...
|---|---|---|---|
| **► THE REGISTER GRID (6 instruments)** | **1b.5 re-run: spread 18.79 → 6.15 dB, and BOTH tutti checks PASS** (−18.9 LUFS, −9.7 dBTP) — RUNNING_LOG §90. His Round Robin change worked: every Xsample velocity curve is monotone now. **The vibraphone is fixed** (+11.66 → −0.85) by measuring its register per bar and letting CC7 carry it. Seven of eight voices within **1.45 dB**. **The last fault is the 3-PITCH GRID itself**, inherited from 0d: the piece uses 13–40 pitches per instrument and most fall OUTSIDE the three measured, where the app clamps to the nearest — **the horn measures 43·50·58 and plays 56–73, so all fourteen pitches above 58 use pitch 58’s data, including every one that sounds through the ReaPitch path.** The cello’s −4.70 and the +1.2–1.45 cluster are both this. **NEXT: a register pass at velocity 127 over the pitches each of the six actually uses, every 2–3 semitones (~62 notes, 7½ min)**, with the velocity shape still from the existing 3×4 data interpolated by register — the vibraphone’s construction, and it is the proof it works. Then rebuild and re-run 1b.5 | Opus | no |
