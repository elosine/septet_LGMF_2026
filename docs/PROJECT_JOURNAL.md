...
|---|---|---|---|
| **► REBUILD, then re-run 1b.5** | **His three are off** (*"done, all three are off"*) — English Horn preset 12, Cello preset 6, D. Bass preset 6, Round Robin off in all four Kontakt slots each, edited IN PLACE so no recipe changes. **`docs/RACK_SETTINGS.md` is now the record of every hand-set plugin value** — the UVI Dynamic parts, the vibraphone’s preset 13, these three — each with what it was, why, and how to revert; it is listed in CLAUDE.md’s orient block. **NEXT, one run then all text:** re-measure english horn · cello · double bass on their curve channels, 3 pitches × 4 velocities (~4 min, one strike each is now exact because they are deterministic); merge; rebuild the remap with the vibraphone’s **every-semitone** register (37 pitches already measured, so no interpolation across pitch at all); check whether any fader needs the quietest-bar reference the vibraphone got; then **re-run 1b.5 unchanged** — its pass is still every part within ±1 dB of −31.84, spread ≤ 3 dB, tutti −20 LUFS-S under −1 dBTP | Opus | no |
