...
|---|---|---|---|
| **► HIS, then 1b.2** | **1b.1 IS BUILT AND PASSES** (RUNNING_LOG §77): the tone reads back −20.000 dBFS / −17.0 LUFS / −17.0 dBTP — the chain is unity and the meter is right; `bank/reference.json`. **TWO THINGS ARE HIS AND CLOSE THE STEP:** (1) play REF's pink noise at **600 s** and set the system volume there once — that is K-20, and a tutti fff at −1 dBTP will sit ~19 dB above it; (2) **CTRL+S** — the REF track, REC's unity fader and three new receives (REF + the two `Horn SI2 high` tracks, which REC had NEVER captured) live only in the open project. **Then 1b.2**, the instrument card: lay out its sub-steps with him first (PLANNING_METHOD), then build | Opus | no |
