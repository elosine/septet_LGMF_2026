...
|---|---|---|---|
| **► HIS CTRL+S, then 1b.4** | **1b.3 IS APPLIED** (RUNNING_LOG §84): **26 tracks, ok: true**, every fader read back within 0.01 dB of what was asked. Every instrument came DOWN 6.6–21.3 dB. **Two tracks were trimmed for the first time ever — `Horn SI2 high` and `Horn SI2 b high`**, the ReaPitch path built after 0d’s hand-typed table; without them the horn above F4 would have stayed 12 dB loud, and five of its six chord notes are up there. `balanceDb` + `bank/perc_rack.json` rewritten to match; `palette_check` 168 · `written_pitch` 10 · `check_ceilings` 6/6 green. **HIS: CTRL+S** — none of it is saved. **THEN 1b.4:** regenerate the velocity remap from `bank/instrument_card.json`, and it must carry the **vibraphone’s per-pitch register offsets** (53 +3.1 · 58 −1.5 · 62 −1.1 · 67 +1.3 · 71 −9.4 · 76 −5.9 · 80 +6.0 · 85 +3.6 · 89 +3.8), which the fader deliberately does not. Decision inside 1b.4: keep the 10 dB written span pp→fff or widen it. Then 1b.5 — the pass is already written: every instrument within ±1 dB of −31.84, tutti −20 LUFS-S under −1 dBTP | Opus | no |
