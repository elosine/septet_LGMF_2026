...
**NOW ►** 2026-09-20 (session 11, Opus → Fable → Opus) — **► PLAN 1e IS BUILT (RUNNING_LOG §143). THE ONE THING LEFT IN IT IS HIS:
record a waved sequence as MIDI in the rack and read it back — `node tools/reaper_job.js run reaper/bridge/jobs/cc7_by_channel.lua`.
Expected: every shaped note on a curve channel · struck at its own REGISTER's mf velocity (the plan's seven numbers are averages over
pitch; a single low note sits below its average — the double bass at C2 is 60) · CC7 reaching down under a deep shape · MAIN ch 1 empty.**
The law is now `docs/DYNAMICS_LAW.md`, the first read for any sound-path work. *What follows is the item as it was written to be built:*
