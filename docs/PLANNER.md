...
**NOW ►** 2026-09-19 (session 6, Fable) — **THE STRIKES DRAWER IS BEING ADAPTED FOR THIS PIECE (PLAN 1c). Stages 1 and 2 are built and verified — the harmony column draws with no strikes, the eight players appear, a fourth set `ordinario` (the vibraphone bowed), and `hear: strike | long tone` with a seconds box: SPACE holds the harmony as dealt, Insert follows — and stage 2b: the drawer's Hear now goes through 1b's remap (it never had), with a `dyn ppp … fff` pull-down on the written scale — and waits for HIS LISTEN** (reload the tab → Strikes → a harmony → `ordinario` → shuffle → `dyn` → `hear: long tone` → SPACE; RUNNING_LOG §92–§95, his request verbatim LG-32). **Stage 3 (1c.3, §96): two vibraphone players — a second seat in the drawer. Stage 4 (1c.4, §97): the HARMONIC SERIES banner — the JUST column: a typed fundamental, every partial to the top key with its cents, Hear bends, Insert writes the bend, fixed-pitch players only within 5 ¢. Stage 5 (1c.5, §99–§100): the range lines on the keys in a colour per instrument, the JUST / 8ve column (green) beside the JUST (pink), the columns measured so nothing overlaps. Stage 6 (1c.6, §101): four rows, each its own harmony — just · just + 8ve · tempered · tempered + 8ve. After his listen: the partial checkboxes (1c.7), the last of LG-32's list** — the SPECTRUM source (a fundamental → just · just per octave · tempered · tempered per octave, cents on the just, partial checkboxes, set toggles) · range lines · takes — under his constraints: nothing existing changed unless necessary and checked with him, only what he asks, robust. **Before that, still standing:** PLAN 1a AND PLAN 1b ARE BOTH CLOSED. SIX SCORES EXIST AND NONE HAS BEEN HEARD SINCE THE RACK CHANGED. 1a built the harmony: `lgmf-ref` (the six chords standing still, 6:50), the four transition scores (9:50 each) and `lgmf-all` (all 24, chord-major, 39:50), every transition reopenable in the morph panel on its own dials. 1b then calibrated the rack, because his first listens found it *"significantly louder than previous pieces"* and clipping: an absolute K-20 target, a proven meter and a −20 dBFS reference track, every instrument re-measured on the channels the piece plays, new trims, and a remap at a **12 dB written span** (was ~10). Measured end to end through the app’s own path: tutti fff **−19.1 LUFS**, **−9.7 dBTP**, per-part spread **7.71 dB** — closed there at his word, *"it is realistic that it is going to vary a bit."* **► HIS LISTENS are the only thing outstanding** — reload the composer tab first; the routing bug of §75, four round robins, the SI2 Dynamic fix and every trim have all changed how these sound. **After that the work is a DESIGN conversation, not a build — how the six chords are used in TIME**: their order, their durations, what happens between them, and whether they are the morph refrain’s stations (LG-6, LG-8) or the opening’s harmony (LG-15). The LGMF call is still unread at his word.
