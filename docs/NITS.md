...
- **2026-09-19 — three stray UVI `Dynamic` values on parts the piece does not play** (RUNNING_LOG §85): `Bassoon SI2` part 2 (Blow Without Reed) reads **0.54** and part 4 (Cresc & Decresc KS) **0.50**; `Trumpet SI2 b` part 1 (Slap Pitched) reads **0.95**. Everything else across the three SI2 instruments is the factory 0.70, with the Ordinario parts the piece plays now at 1.00. Harmless until one of those techniques is used — but any technique brought into the piece needs its Dynamic checked on BOTH the main part and its curve copies, which is the whole lesson of §85.
