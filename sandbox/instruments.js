...
const INSTRUMENTS = {
  ...
  // doubling a technique. Nothing is registered yet: he has named ONE instrument, and it is the
  // one library that is missing. `main` is the plain struck voice (the registry's own key, as
  // #5's piano used it) — a placeholder so the lane can hold material and be laid out.
  // Range 21–108 so `laneCanPlay` never routes material away from the percussionist while the
  // real instruments are unknown; it is NOT a claim about any instrument's compass.
  percussion: {
