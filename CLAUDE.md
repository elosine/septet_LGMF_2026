...
per-instrument table in the app, the percussion selection applied) · `node tools/test_written_pitch.js` (**10** + a control) · `node tools/spectrum_check.js` (**35** — the harmonic series arithmetic behind the strikes drawer's HARMONIC SERIES banner, PLAN 1c.4) · `node tools/sequence_check.js` (**88** — the sequence generator on the six reference chords: both change rules, the dynamic carry, the ceilings, one seed, the refusals, a REST under both rules, THE GATE against `tools/sequence_baseline.json`, the breath dials; PLAN 1d.1 · 1d.4 · 1d.5). Run the first three
